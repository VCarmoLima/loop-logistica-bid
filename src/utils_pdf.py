...
class PDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 10, f'Página {self.page_no()} - Documento Gerado Automaticamente', 0, 0, 'C')
...
def gerar_pdf_auditoria_completo(bid, lances, vencedor_escolhido, rankings):
    ...
    pdf.cell(120, 6, f"Modelo: {bid['titulo']}", 0, 1)
    ...
    if vencedor_escolhido:
        pdf.set_font('Arial', 'B', 10)
        pdf.ln(2)
        pdf.cell(0, 6, f"Transportadora: {vencedor_escolhido['transportadora_nome']}", 0, 1)
        pdf.set_font('Arial', '', 10)
        pdf.cell(0, 6, f"Valor Fechado: R$ {vencedor_escolhido['valor']:,.2f}", 0, 1)
        pdf.cell(0, 6, f"Prazo de Entrega: {vencedor_escolhido['prazo_dias']} dias", 0, 1)
        pdf.cell(0, 6, f"Homologado em: {datetime.now().strftime('%d/%m/%Y %H:%M')}", 0, 1)
    else:
        pdf.cell(0, 6, "Nenhum vencedor selecionado (Leilão Deserto ou Cancelado).", 0, 1)
    ...
    max_rows = max(len(rankings['preco']), len(rankings['prazo']), len(rankings['score']))
    ...
    for i in range(max_rows):
        ...
        y_atual = pdf.get_y()
        pdf.multi_cell(col_w, 5, txt, 1, 'C')
        pdf.set_xy(x_atual + col_w, y_atual) # Volta pro início da próxima linha? Não, vai pro fim.
